...
app.get('/health', (req, res) => {
  res.json({ status: 'ok', service: 'HisaabKitaab Pakistan API', timestamp: new Date().toISOString() });
});
...
app.get('/', (req, res) => {
  res.json({
    status: 'online',
    service: 'HisaabKitaab Pakistan API',
    version: '1.0.0',
...
async function start() {
  try {
    ...
    app.listen(PORT, async () => {
      console.log(`\n🚀 HisaabKitaab Pakistan API running on http://localhost:${PORT}`);
