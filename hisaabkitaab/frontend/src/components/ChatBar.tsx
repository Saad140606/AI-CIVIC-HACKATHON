...
export default function ChatBar() {
  ...
  const [messages, setMessages] = useState<Message[]>(() => {
    try {
      const saved = localStorage.getItem('hisaabkitaab_chat_history');
      return saved ? JSON.parse(saved) : [
        {
          role: 'ai',
          text: `👋 Assalam-o-Alaikum! I'm HisaabKitaab AI. Ask me anything about Pakistan's federal budget or MNAs — in English, Urdu, or Roman Urdu!\n\nTry: "Which ministry got the most money?", "sehat ka budget kitna hai" (Roman Urdu), or "تعلیم کو کتنا پیسہ ملا؟"`,
        },
      ];
    } catch {
      return [
  ...
  useEffect(() => {
    try {
      localStorage.setItem('hisaabkitaab_chat_history', JSON.stringify(messages.slice(-20)));
    } catch {}
  }, [messages]);
            ...
            >
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => {
                    setMessages([]);
                    localStorage.removeItem('hisaabkitaab_chat_history');
                  }}
