...
export default function ShareCard({ ministry, changePercent, mna, taxData, lang = 'en' }: Props) {
  ...
  const handleShare = useCallback(async () => {
    ...
    try {
      ...
      canvas.toBlob(blob => {
        ...
        a.href = url;
        const filename = mna
          ? `hisaabkitaab-mna-${mna.name.replace(/\s+/g, '-').toLowerCase()}.png`
          : taxData
          ? `hisaabkitaab-tax-breakdown.png`
          : `hisaabkitaab-budget-${ministry?.ministry.replace(/\s+/g, '-')}.png`;
        a.download = filename;
        a.click();
  ...
  const handleDownloadPDF = useCallback(async () => {
    ...
    try {
      ...
      const filename = mna
        ? `hisaabkitaab-mna-${mna.name.replace(/\s+/g, '-').toLowerCase()}.pdf`
        : taxData
        ? `hisaabkitaab-tax-breakdown.pdf`
        : `hisaabkitaab-budget-${ministry?.ministry.replace(/\s+/g, '-')}.pdf`;
      pdf.save(filename);
    } catch (err) {
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 24, justifyContent: isUrdu ? 'flex-end' : 'flex-start' }}>
            ...
            <div style={{ textAlign: isUrdu ? 'right' : 'left' }}>
              <div style={{ fontWeight: 700, fontSize: 14, color: '#00D4FF' }}>HisaabKitaab Pakistan</div>
              <div style={{ fontSize: 11, color: '#8892A4' }}>حساب کتاب</div>
            </div>
          </div>
          ...
          <div style={{ paddingTop: 16, borderTop: '1px solid #1E3A5F', fontSize: 12, color: '#4A5568', display: 'flex', justifyContent: 'space-between', flexDirection: isUrdu ? 'row-reverse' : 'row' }}>
            <span>hisaabkitaab.pk • #WakalaCheck</span>
      ...
      <div className="w-full">
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 24, justifyContent: isUrdu ? 'flex-end' : 'flex-start' }}>
            ...
            <div style={{ textAlign: isUrdu ? 'right' : 'left' }}>
              <div style={{ fontWeight: 700, fontSize: 14, color: '#c084fc' }}>HisaabKitaab Pakistan</div>
          ...
          <div style={{ paddingTop: 16, borderTop: '1px solid #7c3aed22', fontSize: 12, color: '#4A5568', display: 'flex', justifyContent: 'space-between', flexDirection: isUrdu ? 'row-reverse' : 'row' }}>
            <span>hisaabkitaab.pk • #TaxCalculator</span>
    ...
    <div>
      ...
      >
        ...
        <div style={{ marginTop: 32, paddingTop: 16, borderTop: '1px solid #1E3A5F', fontSize: 12, color: '#4A5568' }}>
          hisaabkitaab.pk • #PakistanBudget2526 • #حساب_کتاب
        </div>
      </div>
